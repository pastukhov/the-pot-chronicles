...
def env_or_exit() -> tuple[str, str, str | None]:
  ...
  if not assistant_id:
    ...
    raise SystemExit(1)
  project_id = os.getenv(PROJECT_ENV, "").strip() or None
  return api_key, assistant_id, project_id
...
def request_json(
  method: str, url: str, api_key: str, params: Dict[str, Any] | None = None, project_id: str | None = None
) -> Dict[str, Any]:
  headers = {"OpenAI-Beta": "assistants=v2", "Authorization": f"Bearer {api_key}"}
  if project_id:
...
def list_threads(api_key: str, assistant_id: str, project_id: str | None) -> List[Dict[str, Any]]:
  ...
  while True:
    ...
    payload = request_json("GET", url, api_key, params=params, project_id=project_id)
...
def list_messages(api_key: str, thread_id: str, project_id: str | None) -> List[Dict[str, Any]]:
...
def main() -> int:
  api_key, assistant_id, project_id = env_or_exit()
  ensure_dirs()

  threads = list_threads(api_key, assistant_id, project_id)
  ...
  for thread in threads:
    ...
    if path.exists():
      skipped += 1
      continue
    messages = list_messages(api_key, tid, project_id)

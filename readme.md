...
## Локальный запуск
Требуется Python 3.12+ и переменная окружения `OPENAI_API_KEY`.

```bash
pip install -r scripts/requirements.txt
python scripts/fetch_chats.py
...
- `.github/workflows/deploy.yml` — собирает Hugo-сайт и публикует на GitHub Pages при пуше в `main`.
